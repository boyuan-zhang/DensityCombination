...
%% Shrinkage setting
ngrid = 10;
grid_lam_2 = [linspace(1e-15, 0.2, ngrid/2)';linspace(0.3, 20, ngrid/2)']; % Dirichlet shrinkage
grid_lam_3 = [linspace(1e-15, 10, ngrid/2)';linspace(15, 10000, ngrid/2)']; % L2 shrinkage
...
parfor simulind = 1:nsimul
    ...
    for t = 1:(T+T2)
        ...
        xt(t,1) = x0;
        st(t,:) = lam_s'*x0 + sig_s*randn(1,K); %lam_s = 1 in this dgp.
        yt(t,1) = x0 + sig_y*randn(1,1);
    end
    
    % height of the density
    ft = normpdf(yt, st, sig_y);
    ...
    n_reg3 = sum(temp_b_reg3>n_cut,1);
    ...
    % Averaging methods - Best N<=Nmax-Mixture
...
%% Save
cd(resultpath)
savefilename = ['dgp1_K',num2str(K), 'T',num2str(T), '.mat'];
save(savefilename);
cd(workpath)
...
plot(res_m1(2)*ones(ngrid,1), 'linewidth', 3, 'color','black')
hold on
plot(res_m1(1)*ones(ngrid,1), 'linewidth', 3, 'color','black')
plot(res_m2, 'linewidth', 3, 'color','black')
plot(res_m3, 'linewidth', 3, 'color','black')
hold off
ylim([1.12, 1.36]); yticks(1.12:0.04:1.36);
xlim([0.5,ngrid+0.5]); xticks(1:1:10);
xlabel('Penalty Strength');
ylabel('Average Log Score');

text(3,1.145,'Simple Average (and N-Average)', 'fontsize', 20)
text(1,1.21,{'Simplex \newline+Entropy'}, 'fontsize', 20)
text(3,1.26,{'Simplex \newline+Ridge'}, 'fontsize', 20)
text(2,1.34,'Simplex', 'fontsize', 20)
...
title('DGP 1','FontWeight','normal','FontSize',22);


cd(savepath);
saveas(fig,['fig_simul_dgp1_part1_K',num2str(K), '_T',num2str(T),'.png']);
cd(workpath);
...
title(['DGP 1 (T=',num2str(T),')']);
grid on

cd(savepath);
saveas(fig,['fig_simul_dgp1_part2_K',num2str(K), '_T',num2str(T),'.png']);
cd(workpath);
...
write(output_table, ['simul_result2plot_dgp1_part1_K',num2str(K), '_T',num2str(T),'.csv'])
...
write(output_table, ['simul_result2plot_dgp1_part2_K',num2str(K), '_T',num2str(T),'.csv'])
...
fid = fopen([latexpath '/table_DGP1_ls_all_K',num2str(K), '_T',num2str(T),'.tex'],'wt');
...
fprintf(fid, '%s ', 'Simple $K$-Average');
fprintf(fid, '%s ', '&');
