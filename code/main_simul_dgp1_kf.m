...
%     ft2 = normpdf(yt, st, sig_y); % original st = zt
...
%% Save
cd(resultpath)
savefilename = ['dgp1_K',num2str(K), 'T',num2str(T), '_KF.mat'];
save(savefilename);
cd(workpath)
...
cd(savepath);
saveas(fig,['fig_simul_dgp1_part1_K',num2str(K), '_T',num2str(T),'_KF.png']);
cd(workpath);
...
cd(savepath);
saveas(fig,['fig_simul_dgp1_part2_K',num2str(K), '_T',num2str(T),'_KF.png']);
cd(workpath);
...
write(output_table, ['simul_result2plot_dgp1_part1_K',num2str(K), '_T',num2str(T),'_KF.csv'])
...
write(output_table, ['simul_result2plot_dgp1_part2_K',num2str(K), '_T',num2str(T),'_KF.csv'])
...
fid = fopen([latexpath '/table_DGP1_ls_all_K',num2str(K), '_T',num2str(T),'_KF.tex'],'wt');
