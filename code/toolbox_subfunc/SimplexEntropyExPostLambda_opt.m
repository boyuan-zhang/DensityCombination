function [score_final,n_final,b_final] = SimplexEntropyExPostLambda_opt(xxx, w, ns0, ns1,score_type, lam)
...
for t = ns0:ns1
    ...
    % estimation
    if score_type == 'R'
        fun = @(b) sum(rps_tw(xxx(t0:t1),b)) - lam*sum(log(b));
    elseif score_type == 'B'
        fun = @(b) sum(bs_tw(xxx(t0:t1),b)) - lam*sum(log(b));
    elseif score_type == 'L'
        fun = @(b) sum(ls_tw(xxx(t0:t1),b)) - lam*sum(log(b));
    end
