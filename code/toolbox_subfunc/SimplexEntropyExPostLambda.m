function score_final = SimplexEntropyExPostLambda(xxx, w, ns0, ns1,score_type, lam)
...
for t = ns0:ns1
    ...
    % estimation
    if score_type == 'R'
        fun = @(b) sum(rps_tw(xxx(t0:t1),b)) - lam*sum(log(b)); %we are solving min problem
    elseif score_type == 'B'
        fun = @(b) sum(bs_tw(xxx(t0:t1),b)) - lam*sum(log(b)); %we are solving min problem
    elseif score_type == 'L'
        fun = @(b) sum(ls_tw(xxx(t0:t1),b)) - lam*sum(log(b)); %we are solving min problem
    end
...
score_final = mean(mat_score_f(ns0:ns1)); %we are solving min problem
n_final= mat_score_n;
