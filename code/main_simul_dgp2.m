...
%% Save
cd(resultpath)
savefilename = ['dgp2_K',num2str(K), 'T',num2str(T), '.mat'];
save(savefilename);
cd(workpath)
...
hold off
ylim([1.1, 1.8]); yticks(1.1:0.1:1.8);
xlim([0.5,ngrid+0.5]); xticks(1:1:10);
xlabel('Penalty Strength');
ylabel('Average Log Score');

text(2,1.68,'Simple Average', 'fontsize', 20)
text(6.8,1.43,{'Simplex \newline+Entropy'}, 'fontsize', 20)
text(5,1.41,{'Simplex \newline+Ridge'}, 'fontsize', 20)
text(8.5,1.36,'Simplex', 'fontsize', 20)
text(8.5,1.22,'N-Average', 'fontsize', 20)
...
cd(savepath);
saveas(fig,['fig_simul_dgp2_part1_K',num2str(K), '_T',num2str(T),'.png']);
cd(workpath);
...
cd(savepath);
saveas(fig,['fig_simul_dgp2_part2_K',num2str(K), '_T',num2str(T),'.png']);
cd(workpath);
...
write(output_table, ['simul_result2plot_dgp2_part1_K',num2str(K), '_T',num2str(T),'.csv'])
...
write(output_table, ['simul_result2plot_dgp2_part2_K',num2str(K), '_T',num2str(T),'.csv'])
...
fid = fopen([latexpath '/table_DGP2_ls_all_K',num2str(K), '_T',num2str(T),'.tex'],'wt');
